...
def calculate_metrics(logits: torch.Tensor, attention_scores: torch.Tensor, num_gen_tokens: int) -> Dict[str, torch.Tensor]:
    entropy, varentropy = calculate_varentropy_logsoftmax(logits)

    # NB chua: filter to non-zero values because future values are always zero (causal mask)
    attention_scores = attention_scores[:, :num_gen_tokens, :num_gen_tokens]
...
def vanilla_sample(gen_tokens: torch.Tensor, logits: torch.Tensor, attention_scores: torch.Tensor, **kwargs) -> torch.Tensor:
...
def sample(gen_tokens: torch.Tensor, logits: torch.Tensor, attention_scores: torch.Tensor,
    ...
    metrics = calculate_metrics(logits, attention_scores, len(gen_tokens))
